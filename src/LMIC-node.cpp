 ...
 *  File:          LMIC-node.cpp
 * 
 *  Function:      LMIC-node main program file.
 * 
 *  Author:        Curtis Burke
 * 
 *  Company:       IES Industries, LLC
 * 
 *  Description:   To get Heltec ESP32 wifi + LoRa V2 up and running as a 
 *                 LMIC-node, only key configuartion and sensor information needs to 
 *                 be added/change in this source file. 
 *                      Configuration is required in lorawan-keys.h and sensors.h.
 *                      Define Sensors (around line 60)
 *                      Pass all sensors to gatherData();   (around line 685)
 *                      Initialize all sensors in setup section (around line 800)
 * 
 ...
 *                 global_vars.h
 *                 sensors.h
 *                 lorawan-key.h            - included in LMIC-node.h file
 * 
...
lmic_tx_error_t scheduleUplink(uint8_t fPort, uint8_t* data, uint8_t dataLength, bool confirmed = false)
{
    ...
    lmic_tx_error_t retval = LMIC_setTxData2(fPort, data, dataLength, confirmed ? 1 : 0);               // **************** This is the line that actually makes the send via LoRa ************************
...
void processWork(ostime_t doWorkJobTimeStamp)
{
    ...
    // Prepare uplink payload. (LPP format)
    gatherData(pressureSensor);                 // Pass All sensors here (edit the function definition in sensors.cpp)
    

    // Display Data (Serial & Display)
    #ifdef USE_DISPLAY
        ...
        display.print("s");        
        //display.print(" Raw:");
        //display.print(pressureSensor.RawValue);
    #endif
    #ifdef USE_SERIAL
        printEvent(timestamp, "Input data collected", PrintTarget::Serial);
        printSpaces(serial, MESSAGE_INDENT);
        //serial.print(F("Payload Buffer: "));
        //serial.println(*payloadBuffer.getBuffer());
        serial.println();
    #endif    
    ...
    if (LMIC.devaddr != 0)
    {
        ...
        else
        {
            ...
            scheduleUplink(fPort, payloadBuffer,payloadBufferLength);
        }
    }
}    
...
void setup() 
{
    ...
    // -----------------------------------------------------------------------------------------------------
    // Initialize Sensors
    pressureSensor.SensorInit(1,0,13);
